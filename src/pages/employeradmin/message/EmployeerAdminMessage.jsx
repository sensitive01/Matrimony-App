...
// export default EmployeerAdminMessage;
...
const EmployeerAdminMessage = () => {
  ...
  const fetchChats = async (employerAdminId) => {
    try {
      ...
      // Enhance chat data with employee info and unread counts
      const enhancedChats = await Promise.all(
        response.data.data.map(async (chat) => {
          try {
            // Fetch employee data
  ...
  const markMessagesAsRead = async (employeeId, jobId) => {
    try {
      await axios.post(
        `${VITE_BASE_URL}/employer/mark-read`,
        {
          employeeId,
          employerId: employerAdminData._id,
          jobId,
