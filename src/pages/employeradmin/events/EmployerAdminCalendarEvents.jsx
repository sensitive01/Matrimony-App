...
const EmployerAdminCalendarEvents = () => {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/employer/geteveent?employerId=${employerAdminData._id}`
      );
      if (!response.ok) {
  ...
  const deleteEvent = async (eventId) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/employer/deletecalendarevent/${eventId}?employerId=${employerAdminData._id}`,
        {
          method: "DELETE",
        }
      );
...
export default EmployerAdminCalendarEvents;
