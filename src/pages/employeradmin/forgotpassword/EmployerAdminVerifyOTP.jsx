...
// export default EmployerAdminVerifyOTP;
...
const EmployerAdminVerifyOTP = () => {
  ...
  const showOtpToast = (otpValue, isNew = false) => {
    toast.info(
      <div>
        <div>{isNew ? 'New ' : ''}Development OTP: <strong>{otpValue}</strong></div>
  ...
  useEffect(() => {
    ...
    if (sessionOtp) {
      ...
      showOtpToast(sessionOtp);
      sessionStorage.removeItem('tempOtp'); // Remove after showing
    } 
    // Only check location.state if no session OTP
   ...
   const handleResendOTP = async () => {
    ...
    try {
      ...
      if (response.message === "OTP sent successfully") {
        ...
        setOtp(['', '', '', '']);
        setCurrentOtp(response.otp); // Update current OTP
        showOtpToast(response.otp, true); // Show new OTP toast
        
        // Store in sessionStorage instead of location state
        sessionStorage.setItem('tempOtp', response.otp);
