...
export const sendInterestData = async (interestData,userId) => {
  const response = await userInstance.post(`/show-user-interests/${userId}`,{interestData});
  return response;
};


export const getInterestedProfile = async (userId,reqStatus) => {
  const response = await userInstance.post(`/get-interested-profile-request/${userId}`,{reqStatus});
  return response;
};

export const handleChangeInterestStatus = async (userId,profileId,reqStatus) => {
  const response = await userInstance.put(
    `/change-interest-status/${userId}`,
    { reqStatus ,profileId}
  );
  return response;
};